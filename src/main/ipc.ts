...
export function registerCoachIpc(mainWindow: BrowserWindow, wake: WakeRuntime | null): void {
  ...
  async function runSkillPipeline(text: string, withScreenshot: boolean): Promise<void> {
    ...
    const skillId = classifyIntentHeuristic(text);
    const skill = getSkill(skillId);
    if (!skill) {
      mainWindow.webContents.send(CHANNELS.coachError, `skill "${skillId}" 未注册`);
      return;
    }
    log.info('skill.route', { skillId, textLen: text.length, withScreenshot });
    ...
    void (async () => {
      try {
        for await (const chunk of await synthesizeSpeechStream(output.reply)) {
          mainWindow.webContents.send(CHANNELS.voiceTtsChunk, Buffer.from(chunk).toString('base64'));
        }
